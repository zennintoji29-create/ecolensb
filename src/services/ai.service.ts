...
const PLASTIC_CLASSES: PlasticType[] = ['PET', 'HDPE', 'PVC', 'LDPE', 'PP', 'PS', 'Other']

// Biodegradable probabilities per class (scientific estimates)
const BIO_PROBS: Record<PlasticType, number> = {
  PET:   0.12,
  HDPE:  0.10,
  PVC:   0.05,
  LDPE:  0.10,
...
export async function loadModel(): Promise<void> {
  ...
  modelReady = true
  logger.info('AI model loaded (mock mode — swap ai.service.ts for TF.js production model)')
}

export async function predict(imageBuffer: Buffer): Promise<PredictionResult> {
  if (!modelReady) {
  ...
  const rawScores = PLASTIC_CLASSES.map((_, i) =>
    i === index ? confidence : Math.abs(Math.sin(imageBuffer[i % imageBuffer.length] + i)) * 0.08
  )

  logger.info('Mock prediction complete', { plasticType, confidence: confidence.toFixed(3) })
