import { PlasticType, EcoGuidance, RecyclabilityLabel } from '../types/scan.types'
...
const GUIDANCE_MAP: Record<PlasticType, EcoGuidance> = {
  PET: {
    ...
    environmentalImpact:
      'PET bottles take approximately 450 years to decompose in landfill and release harmful microplastics into soil and groundwater over time.',
  ...
  HDPE: {
    ...
    environmentalImpact:
      'HDPE is one of the most recyclable plastics, yet only 30% is actually recycled. When landfilled, it persists for over 100 years.',
    recyclabilityLabel: 'Widely Recyclable',
  ...
  PVC: {
    ...
    environmentalImpact:
      'PVC contains harmful additives (phthalates, lead) that leach into the environment. It is very rarely recyclable through mainstream channels.',
  ...
  LDPE: {
    ...
    environmentalImpact:
      'LDPE film is a major contributor to ocean pollution as it is lightweight and easily airborne. It can harm marine wildlife when ingested.',
  ...
  PP: {
    ...
    environmentalImpact:
      'Polypropylene is increasingly recyclable but collection infrastructure is inconsistent. In landfill, it breaks down into microplastics within decades.',
    recyclabilityLabel: 'Check Locally',
  ...
  PS: {
    ...
    environmentalImpact:
      'Polystyrene foam breaks into tiny beads that persist in the environment for over 500 years and are mistaken for food by wildlife and marine animals.',
  ...
  Other: {
    ...
    environmentalImpact:
      'Mixed or unidentified plastics are typically not recyclable through mainstream channels and persist in the environment for unknown durations.',
    recyclabilityLabel: 'Not Recyclable',
    decompositionYears: 'Unknown',
  },
}

export function getGuidance(plasticType: PlasticType): EcoGuidance {
  const guidance = GUIDANCE_MAP[plasticType]
  if (!guidance) {
    throw new AppError(`No guidance found for plastic type: ${plasticType}`, 500, 'GUIDANCE_NOT_FOUND')
  }
  return guidance
}

export function getRecyclabilityLabel(plasticType: PlasticType): RecyclabilityLabel {
